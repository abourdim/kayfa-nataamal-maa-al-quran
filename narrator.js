/* ═══════════════════════════════════════════════════════════
   NARRATOR.JS — Audiobook narrator for Kayfa Nataamal maa al-Quran
...
(function() {
  ...
  function setupMediaSession() {
    ...
    var l = getLang();
    navigator.mediaSession.metadata = new MediaMetadata({ title: l === 'ar' ? 'كيف نتعامل مع القرآن' : l === 'fr' ? 'Comment Aborder le Coran' : 'How to Deal with the Quran', artist: l === 'ar' ? 'الشيخ محمد الغزالي' : 'Sheikh Mohammed al-Ghazali', album: STATE.mode === 'book' ? (l === 'ar' ? 'الكتاب كاملاً' : l === 'fr' ? 'Livre complet' : 'Full Book') : getActiveTabName() });
  ...
  function onSpeedChange(val) { STATE.speed = parseFloat(val); var label = document.getElementById('narratorSpeedLabel'); if (label) label.textContent = STATE.speed + 'x'; localStorage.setItem('kq-narrator-speed', STATE.speed); }
  function onPitchChange(val) { STATE.pitch = parseFloat(val); var label = document.getElementById('narratorPitchLabel'); if (label) label.textContent = STATE.pitch.toFixed(1); localStorage.setItem('kq-narrator-pitch', STATE.pitch); }
  ...
  function onKaraokeToggle(checked) { STATE.karaokeEnabled = checked; localStorage.setItem('kq-narrator-karaoke', checked); }
  function onAutoScrollToggle(checked) { STATE.autoScroll = checked; localStorage.setItem('kq-narrator-autoscroll', checked); }
  function onDuoToggle(checked) { STATE.duoReading = checked; localStorage.setItem('kq-narrator-duo', checked); }
  ...
  function loadSettings() { STATE.speed = parseFloat(localStorage.getItem('kq-narrator-speed')) || 1; STATE.pitch = parseFloat(localStorage.getItem('kq-narrator-pitch')) || 1; STATE.karaokeEnabled = localStorage.getItem('kq-narrator-karaoke') !== 'false'; STATE.autoScroll = localStorage.getItem('kq-narrator-autoscroll') !== 'false'; STATE.duoReading = localStorage.getItem('kq-narrator-duo') === 'true'; }
